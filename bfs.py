...
# Function to find the corresponding BFS output file name
def get_corresponding_bfs_file(input_directory, output_directory):
    ...
    return f"{output_directory}/bfs_{maze_index}.csv"
...
def bfs(grid, start, goal, screen, CELL_SIZE, MARGIN, quit_button_rect):
    ...
    while queue:
        ...
        if current not in visited:
            visited.add(current)
            grid[current[0]][current[1]] = 5  # Mark visited (light blue)
            display_maze(grid, screen, CELL_SIZE, MARGIN, quit_button_rect)
            time.sleep(0.02)  # Slow down animation
...
# Main function to execute BFS and visualize the result
def main(input_directory, output_directory):
    ...
    if path:
        ...
        output_file = get_corresponding_bfs_file(input_directory, output_directory)
...
if __name__ == "__main__":
    ...
    output_directory = r"C:\Users\prudhvi\OneDrive\Desktop\Afrid\maze project\maze final\mazes_output\bfs"  # Path to save output CSV file
